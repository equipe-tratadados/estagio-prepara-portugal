...
setup_git() {
    ...
    if command -v git-lfs &> /dev/null; then
        ...
        # Configura extensões
        git lfs track "*.csv" 2>/dev/null
        git lfs track "*.parquet" 2>/dev/null
        git lfs track "*.pkl" 2>/dev/null
        git lfs track "*.joblib" 2>/dev/null
        git lfs track "*.h5" 2>/dev/null
        git lfs track "*.feather" 2>/dev/null
        git lfs track "*.arrow" 2>/dev/null
...
setup_pr_template() {
    echo_title "📋 CONFIGURANDO TEMPLATE DA PR"
...
- [ ] Menos de 400 linhas alteradas

## 🔗 Dependencies

<!-- Links para PRs dependentes, se houver -->
EOF
...
update_gitignore() {
    ...
    if ! grep -q ".hooks/logs/" .gitignore 2>/dev/null; then
        echo "" >> .gitignore
        echo "# Git hooks - logs e backups" >> .gitignore
...
show_welcome() {
    ...
    echo -e "${BLUE}╔════════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║                                                                    ║${NC}"
    echo -e "${BLUE}║  ${BOLD}🚀 INSTALADOR UNIVERSAL DE HOOKS PARA CIÊNCIA DE DADOS${NC}    ║${NC}"
    echo -e "${BLUE}║                                                                    ║${NC}"
